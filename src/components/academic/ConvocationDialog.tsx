...
const ConvocationDialog: React.FC<ConvocationDialogProps> = ({ isOpen, onClose, studentData }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="relative w-full max-w-2xl bg-white text-slate-950 shadow-2xl rounded-sm overflow-hidden flex flex-col max-h-[90vh]">
        <div className="absolute top-4 right-4 flex gap-2 no-print">
          <Button variant="outline" size="icon" onClick={() => window.print()} className="rounded-full">
        ...
        <div id="printable-convocation" className="p-12 space-y-8 overflow-y-auto">
          {/* Header */}
          ...
          <div className="text-center space-y-4 py-4">
            ...
            <p className="text-lg italic font-serif">Projet de Fin d'Études (PFE)</p>
          </div>

          {/* Body */}
          <div className="space-y-6 font-serif text-lg leading-relaxed">
            <p>
              Il est porté à la connaissance de l'étudiant(e) <span className="font-bold underline">{studentData.name}</span> (CNE: {studentData.cne}), 
              inscrit(e) en <span className="font-bold">{studentData.filiere}</span>, que sa soutenance est programmée pour :
            </p>
          ...
          <div className="pt-12 flex justify-between items-end">
            <div className="text-[10px] text-slate-400 max-w-[200px]">
              Ce document est généré électroniquement et tient lieu de convocation officielle.
            </div>
            <div className="text-center">
              <p className="text-sm font-bold uppercase underline">Le Coordonnateur</p>
              <div className="h-16 w-32 mx-auto flex items-center justify-center italic text-slate-300">
                Cachet de la Faculté
              </div>
            </div>
          </div>
        </div>
