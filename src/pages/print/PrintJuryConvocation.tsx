...
export default function PrintJuryConvocation() {
  ...
  const { data: convocations, isLoading, error } = useJuryConvocations(projectId ? Number(projectId) : null);
  ...
  const items = Array.isArray(convocations) ? convocations : [convocations];
  const conv = items.find((c) => c.teacherName.includes(teacherId));
  ...
  return (
    <div data-testid="print-jury-root">
      <PrintLayout title="Convocation — Membre du jury" autoPrint>
        <JuryConvocation
